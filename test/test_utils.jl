...
function test_utils()
    ...
    @testset "subs2" begin
        ...
        @testset "scalar indexing (existing)" begin
            ...
            @test subs2(subs2(e, :x0, :x, 0, 5), :xf, :x, :N, 5) ==
                :(x[1, 0] * (2 * x[3, N]) - cos(x[2, N]) * (2 * x[2, 0]))

            # Test 2: With new pattern, bare symbols ARE substituted to comprehensions
            e = :(x0 * 2xf[3] - cos(xf) * 2x0[2])
            @test subs2(subs2(e, :x0, :x, 0, 5; k = :k1), :xf, :x, :N, 5; k = :k2) ==
                :([x[k1, 0] for k1 ∈ 1:5] * (2 * x[3, N]) - cos([x[k2, N] for k2 ∈ 1:5]) * (2 * x[2, 0]))

            # Test 3: Numeric index
            e = :(x0[5] + x0[10])
            @test subs2(e, :x0, :x, 0, 5) == :(x[5, 0] + x[10, 0])

            # Test 4: Symbolic index
            e = :(x0[i] + x0[j])
            @test subs2(e, :x0, :x, 0, 5) == :(x[i, 0] + x[j, 0])
        end
        ...
        @testset "range indexing (new)" begin
            ...
            e = :(x0[1:3] + xf[2:4])
            result = subs2(subs2(e, :x0, :x, 0, 5; k = :k1), :xf, :x, :N, 5; k = :k2)
        ...
        @testset "nested and complex expressions" begin
            ...
            e = :(x0[1:3] + xf[2:4])
            result = subs2(subs2(e, :x0, :x, :j, 5; k = :k1), :xf, :x, :(j+1), 5; k = :k2)
        ...
        @testset "edge cases" begin
            ...
            e = :(x0[1:3])
            result = subs2(e, :x0, :x, :grid_size, 5; k = :k)
        ...
        @testset "backward compatibility" begin
            ...
            # Test 19: Bare symbols now expand to comprehensions
            e = :(x0 * 2xf[3] - cos(xf) * 2x0[2])
            @test subs2(subs2(e, :x0, :x, 0, 5; k = :k1), :xf, :x, :N, 5; k = :k2) ==
                :([x[k1, 0] for k1 ∈ 1:5] * (2 * x[3, N]) - cos([x[k2, N] for k2 ∈ 1:5]) * (2 * x[2, 0]))
        end
