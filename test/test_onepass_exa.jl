...
# todo: backend = backend to pass all tests on CUDA, test euler(_b)

parsing_backend!(:exa)

e_prefix!(:CTBase) # exceptions in code generated by @def are prefixed by CTBase (not by OptimalControl - the default) for tests

exa_backend = nothing # debug: put exa_backend everywhere
#exa_backend = try CUDABackend() catch _ nothing end # for runners without CUDA 
...
#    end
    ...
    @testset "use case no. 2: Goddard" begin
        ...
        tol = 1e-7
        m = o(; grid_size = N, init = (tfs, xs, us))
        s = madnlp(m; tol = tol)
        @test s.objective ≈ -1.0125736217178989e+00 atol = 1e-10
        #@btime madnlp($m1; tol = $tol)
