...
prefix!(:CTModels) # code generated by @def is prefixed by CTModels (not by OptimalControl - the default) for tests
e_prefix!(:CTBase) # exceptions in code generated by @def are prefixed by CTBase (not by OptimalControl - the default) for tests

function discretise_exa(ocp; log = false) # todo: to be moved into CTDirect
    e = CTModels.definition(ocp)
    m = CTParser.def_exa(e; log = log) 
    return eval(m) # world age! 
end
...
function __test_onepass_exa(backend = nothing)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        @test CTParser.__default_parsing_backend() == :debug # debug
    ...
    @testset "$test_name" begin println(test_name)
        ...
        oo = discretise_exa(o)
        @test oo(; backend = backend) isa ExaModels.ExaModel
    ...
    @testset "$test_name" begin println(test_name)
        ...
        oo = discretise_exa(o)
        @test oo() isa ExaModels.ExaModel 
        @test oo(; backend = backend) isa ExaModels.ExaModel
        @test oo(; grid_size = 100) isa ExaModels.ExaModel
        @test oo(; init = (1., 2., 3., 4., 5.)) isa ExaModels.ExaModel
        @test oo(; base_type = Float32) isa ExaModels.ExaModel
    ...
    @testset "$test_name" begin println(test_name)
       ...
       o = @def begin
        ...
        oo = discretise_exa(o)
        @test_throws ParsingError oo(; backend = backend)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        oo = discretise_exa(o)
        @test_throws String oo(; backend = backend)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        oo = discretise_exa(o)
        @test oo(; backend = backend) isa ExaModels.ExaModel
        @test_throws String oo(; scheme = :foo)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        oo = discretise_exa(o)
        @test oo(; backend = backend) isa ExaModels.ExaModel
        m = oo(; backend = backend)
        s = madnlp(m)
        @test s.objective ≈ 6 atol = 1e-2
        m = oo(; backend = backend, grid_size = 1000)
        s = madnlp(m)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        tol = 1e-7
        oo = discretise_exa(o)
        m = oo(; backend = backend, grid_size = N, init = (tfs, xs, us))
        s = madnlp(m; tol = tol)
    ...
    @testset "$test_name" begin println(test_name)
        ...
        N = 100
        oo = discretise_exa(o)
        m = oo(; grid_size = N, scheme = :euler, backend = backend)
        @test m isa ExaModels.ExaModel
        sol = madnlp(m)
        @test sol.status == MadNLP.SOLVE_SUCCEEDED
        m = oo(; grid_size = N, scheme = :euler_b, backend = backend)
        @test m isa ExaModels.ExaModel
        sol = madnlp(m)
        @test sol.status == MadNLP.SOLVE_SUCCEEDED
