    ...
    @match e begin
        :($a = $b) => @match b begin
            :(($names) ∈ $c) => begin
                ...
                q = Expr(:block)
                q = CTParser.concat(q, quote $a = ($names) ∈ R^$dim, $type end)
                for l ∈ con
        ...
        :($a ∈ $c) => begin
            ...
            con = parse_constraints(p, c, dim, a, type; log=log)
            q = Expr(:block)
            q = CTParser.concat(q, quote $a ∈ R^$dim, $type end)
            for l ∈ con
                q = CTParser.concat(q, l)
            end
            q
...
function parse_constraints(p, e::Union{Symbol,Expr}, dim, label, type; log::Bool=false, idx=dim)
    ...
    @match e begin
        ...
        :($a × $b)  => begin
            k2 = parse_dimension(p, b; log=log)
            e1 = parse_constraints(p, a, dim, label, type; log=log, idx=idx-k2)
            e2 = parse_constraints(p, b, dim, label, type; log=log, idx=idx)
            push!(e1, e2...)
            e1
        end
        _ => return __direct_throw("unknown syntax", p.lnum, p.line)
...
standardise_declaration!(p, quote
    ...
    t ∈ [0, 1], time
    x = (y, z) ∈ R^2, state 
    u = (a, b, c, d, e, f) ∈ R^2 × [-1, 1] × R^2 × R, control
    tf → min
end; log=LOG)
...
q = CTParser.reorder(q)
o = eval(CTParser.def_fun(q)) #; order=true))
