...
function p_lagrange_exa!(p, p_ocp, e, type)
    ...
    code = quote
        if scheme ∈ (:trapeze, :trapezoidal)
            $pref.objective($p_ocp, $sg * $(p.dt) * $ej / 2 for $j in (0, grid_size))
            $pref.objective($p_ocp, $sg * $(p.dt) * $ej for $j in 1:(grid_size - 1))
        elseif scheme == :euler
            $pref.objective($p_ocp, $sg * $(p.dt) * $ej for $j in 0:(grid_size - 1))
        elseif scheme == :euler_b
            $pref.objective($p_ocp, $sg * $(p.dt) * $ej for $j in 1:grid_size)
        else
            throw("unknown numerical scheme: $scheme") # (vs. __throw) since raised at runtime (and __wrap-ped)
        end
    end
...
function def_exa(e; log=false)
    ...
    get_variable =
        :(sol -> isnothing($(p.dim_v)) ? base_type[] : Array($pref.solution(sol, $(p.v)))) # getter for variable from ExaModel solution
    ...
    get_variable_ml = :(
        sol -> if isnothing($(p.dim_v))
            base_type[] # getter for lower variable box multiplier from ExaModel solution
        else
            Array($pref.multipliers_L(sol, $(p.v))) # getter for lower variable box multiplier from ExaModel solution
        end
    ) # getter for lower variable box multiplier from ExaModel solution
    get_variable_mu = :(
        sol -> if isnothing($(p.dim_v))
            base_type[] # getter for upper variable box multiplier from ExaModel solution
        else
            Array($pref.multipliers_U(sol, $(p.v))) # getter for upper variable box multiplier from ExaModel solution
        end
    ) # getter for upper variable box multiplier from ExaModel solution
    code = quote
        function (;
