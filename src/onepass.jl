...
function p_constraint_exa!(p, p_ocp, e1, e2, e3, c_type, label)
    ...
    code = @match c_type begin
        :boundary || :variable_fun => begin
            ...
            e2 = subs2(e2, x0, p.x, 0, p.dim_x)
            e2 = subs2(e2, xf, p.x, :grid_size, p.dim_x)
        ...
        :state_fun || control_fun || :mixed => begin
            ...
            j = __symgen(:j)
            e2 = subs2(e2, xt, p.x, j, p.dim_x)
            e2 = subs2(e2, ut, p.u, j, p.dim_u)
...
function p_dynamics_coord_exa!(p, p_ocp, x, i, t, e)
    ...
    j2 = :($j1 + 1)
    j12 = :($j1 + 0.5)
    ej1 = subs2(e, xt, p.x, j1, p.dim_x)
    ej1 = subs2(ej1, ut, p.u, j1, p.dim_u)
    ej1 = subs(ej1, p.t, :($(p.t0) + $j1 * $(p.dt)))
    ej2 = subs2(e, xt, p.x, j2, p.dim_x)
    ej2 = subs2(ej2, ut, p.u, j2, p.dim_u)
    ej2 = subs(ej2, p.t, :($(p.t0) + $j2 * $(p.dt)))
    ej12 = subs5(e, xt, p.x, j1)
    ej12 = subs2(ej12, ut, p.u, j1, p.dim_u)
...
function p_mayer_exa!(p, p_ocp, e, type)
    ...
    e = replace_call(e, p.x, p.t0, x0)
    e = replace_call(e, p.x, p.tf, xf)
    e = subs2(e, x0, p.x, 0, p.dim_x)
    e = subs2(e, xf, p.x, :grid_size, p.dim_x)
